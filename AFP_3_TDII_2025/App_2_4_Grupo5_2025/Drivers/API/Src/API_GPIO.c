...
/**
  * @brief  Lee el botón USER y, en flanco ascendente, avanza el índice de tiempo (wrap).
  * @param  tiempo_index: puntero al índice actual de tiempos (0..tiempos_count-1).
  * @param  tiempos_count: cantidad de elementos del arreglo de tiempos.
  * @param  debounce_ms: tiempo de antirrebote en milisegundos.
  * @retval None
  */
void APP4_UpdateTimeIndex(uint8_t *tiempo_index, uint8_t tiempos_count, uint32_t debounce_ms)
{
    ...
    {
        if (tiempos_count > 0) {
            *tiempo_index = (uint8_t)((*tiempo_index + 1) % tiempos_count);
        }
        HAL_Delay(debounce_ms); // antirrebote simple
    }
...
/**
  * @brief  Parpadeo simultáneo de todos los LEDs según el tiempo seleccionado por índice.
  * @param  ports: arreglo de puertos de LEDs (LDx_GPIO_Port).
  * @param  pins: arreglo de pines de LEDs (LDx_Pin).
  * @param  led_count: cantidad de LEDs.
  * @param  tiempos: arreglo de periodos (ms) disponibles.
  * @param  tiempo_index: índice actual dentro de 'tiempos'.
  * @retval None
  */
                   ...
                   const uint16_t tiempos[], uint8_t tiempo_index)
{
    ...
    // Protección mínima: si no hay LEDs o no hay tiempos, no hace nada
    if (led_count == 0) return;

    uint32_t periodo = (uint32_t)tiempos[tiempo_index]; // cast para comparar con ticks

    if (now - last_toggle >= periodo)
    {
